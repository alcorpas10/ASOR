...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        v = select(sfd+1, &set, NULL, NULL, NULL);
        if (v == -1) {
        ...
        if (FD_ISSET(sfd, &set)) {
            readsize = recvfrom(sfd, &buf, 2, 0, (struct sockaddr*) &addr, &addrlen);
        } else {
            readsize = read(STDIN_FILENO, &buf, 2);
        }
        if (readsize == -1) {
            perror("Error executing read");
            close(sfd);
            return 1;
        }
